...
/**
 * Indexes newly created image files in real time.
 *
 * @implements IEventListener<NodeCreatedEvent>
 */
class NodeCreatedListener implements IEventListener
{
    ...
    public function handle(Event $event): void
    {
        ...
        try {
            $this->scannerService->processFile($userId, $node);
        } catch (\Throwable $e) {
            $this->logger->error('NodeCreatedListener: failed to index new file', [
                'path' => $node->getPath(),
                'exception' => $e,
            ]);
        }
